cachefile <- "cache/coverage_by_type100.Rdata"
if (!file.exists(cachefile)) {
  ...
  reps <- 1000
  cov.by.type100.save <- makeSaveMatrix(c("truth", "estimate",
                                            "cont_un", "cont_cov",
                                            "disjoint_un",
                                            "disjoint_cov"),
  ...
  for (j in 1:length(true_inter)) {
    ...
    cov.by.type100.save[j,] <- c(ti, mean(save$estimate),
                     as.vector(table(save$covered, save$type)))

  }

  cov.by.type100.save <- as.data.frame(cov.by.type100.save)
  cov.by.type100.save$overall_un <- apply(cov.by.type100.save[names(cov.by.type100.save)[grepl("_un", names(cov.by.type100.save))]], 1, sum)
  cov.by.type100.save$overall_cov <- apply(cov.by.type100.save[names(cov.by.type100.save)[grepl("_cov", names(cov.by.type100.save))]], 1, sum)

  cov.by.type100.save$overall_per <- cov.by.type100.save$overall_cov/(cov.by.type100.save$overall_un + cov.by.type100.save$overall_cov)
  cov.by.type100.save$cont_per <- cov.by.type100.save$cont_cov/(cov.by.type100.save$cont_un + cov.by.type100.save$cont_cov)
  cov.by.type100.save$disjoint_per <- cov.by.type100.save$disjoint_cov/(cov.by.type100.save$disjoint_un + cov.by.type100.save$disjoint_cov)

  cov.by.type100.save <- cov.by.type100.save[,
                         c(1, 2, 7:9, 3:4, 10, 5:6, 11)]

  save(cov.by.type100.save, file = cachefile)
}
