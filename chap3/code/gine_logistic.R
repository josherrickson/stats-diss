cachefile <- "cache/gine_log.Rdata"
if (!file.exists(cachefile)) {
  ...
  # model formula
  mod_form <- fully_paid_sept30 ~ paprikaexp + married + male +
...
# Since NA variables will be dropped anyways, we can ditch them early
  ...
  mod1 <- glm(mod_form, data = final2[final2$fingerprint == 0, ], family=binomial)

  mod1 <- stepAIC(mod1, trace = FALSE)

  final2$pred <- predict(mod1, final2, type="response")

  mod2naive <- lm(fully_paid_sept30 - pred ~ pred,
  ...
  ginecorr_log<- data.frame(Estimate = c(mod2naive$coef[2],
                                      mod2$coef[2],
                                      mod2clus$coef[2]),
  ...
  ginecorr_log$"Confidence Interval" <- paste0("(",
                                          apply(cbind(sprintf("%.3f",
                                                              round(ginecorr_log[,3],3)),
                                                      sprintf("%.3f",
                                                              round(ginecorr_log[,4],3))),
                                                1, paste, collapse = ", "), ")")

  ginecorr_log <- ginecorr_log[,c(1,2,5)]

  row.names(ginecorr_log) <- c("Uncorrected", "Corrected", "Corrected w/ Clusters")

  save(ginecorr_log, file = cachefile)
