cachefile <- "cache/clustersim_1000_100.Rdata"
if (!file.exists(cachefile)) {
  ...
  reps <- 1000
  clus_1000_100 <- pbph:::makeSaveMatrix(c("truth", "estimate", "overall_un",
  ...
  for (j in 1:length(true_inter)) {
    ...
    clus_1000_100[j,] <- c(ti, mean(save$estimate), table(save$covered),
                     as.vector(table(save$covered, save$type)))

  }

  clus_1000_100 <- as.data.frame(clus_1000_100)
  clus_1000_100$perc_cov <- clus_1000_100$overall_cov/reps*100
  save(clus_1000_100, file = cachefile)
}
